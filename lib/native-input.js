...
//     - probe times out         -> the shared pipeline is wedged; the only
//                                  recovery is a browser restart, scheduled
...
export function createStallRecovery({
  ...
  async function afterNativeInputTimeout({ probe, meta = {} } = {}) {
    ...
    let timer = null;
    try {
      await withNativeInput(() => new Promise((resolve, reject) => {
        timer = setTimeout(() => reject(timeoutError), probeTimeoutMs);
        Promise.resolve()
          .then(() => probe())
          .then(resolve, reject);
      }));
      log('warn', 'native input probe completed after timeout', { ...meta, outcome: 'pipeline-alive' });
      ...
      if (err === timeoutError) {
        ...
        if (now - lastRestartAt >= restartCooldownMs) {
          ...
          log('warn', 'native input pipeline wedged, restart already scheduled', {
            ...meta,
            reason: 'input_stall',
          });
        }
