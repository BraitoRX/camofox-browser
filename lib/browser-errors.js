...
export function isStaleVisualCaptureError(err) {
  return err?.code === 'stale_visual_capture';
}
...
export function browserErrorStatus(err) {
  ...
  if (isStaleVisualCaptureError(err)) return 409;
...
export function browserErrorCode(err) {
  ...
  if (isStaleVisualCaptureError(err)) return 'stale_visual_capture';
...
export function browserErrorRecovery(err) {
  ...
  if (code === 'stale_visual_capture') return 'screenshot_then_retry';
